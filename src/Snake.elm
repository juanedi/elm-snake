...
moveFood =
    let
        generator =
            (Random.pair (Random.int 0 (columnCount - 1)) (Random.int 0 (rowCount - 1)))
    in
        Random.generate FoodAppeared generator
    ...
    if model.lost then
        ...
        case msg of
            Advance ->
                ...
                in
                    if lost next model then
                        ( { model | lost = True }, raiseEvent Died )
                    else
                        let
                            bite =
                                next == model.foodPosition
                            ...
                            cmd =
                                if bite then
                                    ...
                                    Cmd.none
                        in
                            ( model |> updateDirection >> moveSnake next, cmd )
