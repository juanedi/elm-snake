...
gridStyle =
    let
        gridWidth =
            Snake.columnCount * Snake.cellSize
    in
        [ ( "width", px gridWidth )
        , ( "padding", "0" )
        ]
...
cellColor model x y =
    if model.lost then
        Color.grey
    else if List.member ( x, y ) model.snake then
        Color.blue
    else if ( x, y ) == model.foodPosition then
        Color.red
    else
        Color.grey
...
cell model rowIndex colIndex =
    let
        cellRgb =
            rgb (cellColor model colIndex rowIndex)

        styles =
            ( "background-color", cellRgb ) :: cellStyle
    in
        div [ style styles ] []


buildRow : Model -> Int -> Html a
buildRow model rowIndex =
    let
        buildCell =
            (\colIndex -> cell model rowIndex colIndex)

        range =
            List.range 0 (Snake.columnCount - 1)

        cells =
            List.map buildCell range
    in
